...
project_objects AS (
    SELECT
        ...
        object_stage_text.text_fi AS object_stage,
        concat((po.reason_for_environmental_investment).id, ' ', env_reason_text.text_fi) AS reason_for_environmental_investment,
        oc.object_category,
        ou.object_usage,
        ot.object_type,
    ...
    FROM
        ...
        LEFT JOIN app.code object_stage_text ON object_stage_text.id = poi.object_stage
        LEFT JOIN app.code env_reason_text ON env_reason_text.id = po.reason_for_environmental_investment
    ...
    WHERE
        ((po.lifecycle_state).id = '01' OR (po.lifecycle_state).id = '02')
...
SELECT
    ...
    pos.object_stage,
    pos.reason_for_environmental_investment,
    pos.object_category,
    pos.object_usage,
    pos.object_type,
    pos.object_geometry,
